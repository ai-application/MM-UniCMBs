...
class UniFormer_Light(nn.Module):
    ...
    def __init__(self, num_classes, depth, prune_ratio, trade_off, embed_dim, head_dim, mlp_ratio, qkv_bias):
        super().__init__()
        ...
        if split:
            self.blocks3 = nn.ModuleList([
                SplitSABlock(
                    dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
            ...
            self.blocks4 = nn.ModuleList([
                SplitSABlock(
                    dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio[2], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
            ...
            self.blocks3 = nn.ModuleList([
                EvoSABlock(
                    dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=mlp_ratio[3], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0] + depth[1]],
                    norm_layer=norm_layer,
            ...
            self.blocks4 = nn.ModuleList([
                EvoSABlock(
                    dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=mlp_ratio[3], qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
                    drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i + depth[0] + depth[1] + depth[2]],
                    norm_layer=norm_layer,
    ...
    def forward_features(self, x):
        ...
        global global_attn, token_indices
        global_attn = 0
        token_indices = torch.arange(x.shape[2] * x.shape[3] * x.shape[4], dtype=torch.long, device=x.device).unsqueeze(
            0)
...
def uniformer_xxs(num_classes):
    model = UniFormer_Light(num_classes = num_classes, depth=[2, 5, 8, 2],
...
def uniformer_xs(num_classes):
    model = UniFormer_Light(num_classes = num_classes, depth=[3, 5, 9, 3],
