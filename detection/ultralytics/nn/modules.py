...
class cmb_mha_layer(nn.Module):
    def __init__(self, input_dim, heads=None, dim_perhead=None, k_size=None):
        ...
        self.Wv = nn.Conv2d(input_dim, input_dim, kernel_size=1)
        self._norm_fact = 1 / sqrt(input_dim / heads)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        b, c, h, w = x.size()
        y_avg_x = self.avg_pool(x)  # [b, c, 1, 1]
        y_avg_x = y_avg_x.squeeze(-1).transpose(-1, -2)  # [b, 1, c]

        y_max_x = self.max_pool(x)  # [b, c, 1, 1]
        y_max_x = y_max_x.squeeze(-1).transpose(-1, -2)  # [b, 1, c]

        Q = self.Wq(y_max_x)  # Q: [b, 1, c]
        K = self.Wk(y_avg_x)  # K: [b, 1, c]
...
class cmb_mha_module(nn.Module):
    dim_perhead = 32

    def __init__(self, input_dim):
        super(cmb_mha_module, self).__init__()
        self.conv = nn.Conv2d(input_dim, input_dim, kernel_size=1)
        self.mha = cmb_mha_layer(input_dim=input_dim, dim_perhead=self.dim_perhead)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = identity + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))
...
class BottleneckCSP(nn.Module):
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        y3 = self.cv4(self.act(self.bn(torch.cat((y1, y2), 1))))
...
class C2f(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cmb_mha = cmb_mha_module(input_dim=c2)
        self.conv3 = Conv(c1, c2)
        self.bn_mha = nn.BatchNorm2d(c2)
    ...
    def forward(self, x):
        ...
        out = out + self.bn_mha(self.cmb_mha(out, self.conv3(identity)))
